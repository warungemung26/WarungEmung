...
function renderCartModal() {
  ...
  const pObj = formatPriceByCurrency(it.price * it.qty, currency);

  const priceStr = pObj.icon
  ...
  //  centralized footer
  const subObj = formatPriceByCurrency(subtotal, currency);
const ongObj = formatPriceByCurrency(ONGKIR, currency);
const totObj = formatPriceByCurrency(total, currency);

const subtotalStr = subObj.icon
  ? `<img src="${subObj.icon}" class="pi-logo"> ${subObj.text}`
  : subObj.text;

const ongkirStr = ongObj.icon
  ? `<img src="${ongObj.icon}" class="pi-logo"> ${ongObj.text}`
  : ongObj.text;

const totalStr = totObj.icon
  ? `<img src="${totObj.icon}" class="pi-logo"> ${totObj.text}`
  : totObj.text;
